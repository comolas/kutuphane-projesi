...
const CollectionModal: React.FC<CollectionModalProps> = ({ onClose, onSave, collectionToEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full h-full md:h-[95vh] md:max-w-6xl flex flex-col overflow-hidden">
        <div className="bg-gradient-to-r from-indigo-500 via-purple-600 to-pink-500 p-6 flex justify-between items-center">
          <h2 className="text-3xl font-bold text-white">{collectionToEdit ? 'Koleksiyonu Düzenle' : 'Yeni Koleksiyon Oluştur'}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto">
          <div className="p-6 space-y-6">
            ...
            {searchResults.length > 0 && (
              <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-2xl p-4">
                <h3 className="font-semibold text-lg mb-4">Arama Sonuçları</h3>
            ...
            {/* Selected Books Grid */}
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-2xl p-4">
              <h3 className="font-semibold text-lg mb-4">Seçili Kitaplar ({selectedBooks.length})</h3>
          ...
          <div className="p-6 border-t bg-gradient-to-r from-gray-50 to-gray-100 flex justify-end space-x-3">
            <button type="button" onClick={onClose} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-xl hover:bg-gray-300">İptal</button>
            <button type="submit" disabled={isLoading} className="px-6 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl hover:shadow-lg disabled:opacity-50">
