...
const ChallengesPage: React.FC = () => {
  ...
  const handleAccept = async (challenge: Challenge) => {
    ...
    Swal.fire('Kabul Edildi!', 'Meydan okuma başladı!', 'success');
    loadChallenges();
  };
  ...
  const handleReject = async (challenge: Challenge) => {
    ...
    Swal.fire('Reddedildi!', 'Meydan okuma reddedildi.', 'info');
    loadChallenges();
  };
  ...
  const challengeLabels = {
    ...
    'category-books': 'Kategori Bazlı',
    'page-count': 'Sayfa Sayısı',
    'reviews': 'Yorum Sayısı',
    'blog-posts': 'Blog Yazısı'
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 py-4 sm:py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">🏆 Meydan Okumalar</h1>
            <p className="text-sm sm:text-base text-gray-600">Arkadaşlarınla yarış, kupon kazan!</p>
          </div>
          <button
                ...
                <div key={challenge.id} className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base sm:text-lg font-bold text-gray-900 truncate">{challengeLabels[challenge.type]}</h3>
                      {challenge.category && (
                        <p className="text-xs sm:text-sm text-gray-600 truncate">Kategori: {challenge.category}</p>
                      )}
                    </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs sm:text-sm font-medium text-gray-700">Sen</span>
                  ...
                  {challenge.status === 'active' && (
                    <div className="flex items-center text-sm text-gray-600 mb-4">
                      ...
                      Bitiş: {challenge.endDate?.toDate().toLocaleDateString('tr-TR')}
                    </div>
                  )}
                  ...
                  {challenge.status === 'completed' && challenge.winnerId && (
                    <div className={`text-center py-2 rounded-lg ${
                      ...
                      {challenge.winnerId === user?.uid ? '🎉 Kazandın!' : '😔 Kaybettin'}
                    </div>
                  )}
                </div>
              );
        ...
        {filteredChallenges.length === 0 && !loading && (
          <div className="text-center py-12 text-gray-500">
            {filter === 'all' ? 'Henüz meydan okuma yok.' : 'Bu durumda meydan okuma yok.'}
          </div>
        )}
      </div>
